...
// Constants
const SPOONACULAR_API_KEY = "0ed538d0d51d4b379a7c794e7b017d5e"; // Replace with your Spoonacular API key
...
app.get('/api/user-preferences', async (req, res) => {
  ...
  const UserRestriction = require("./models/UserRestriction"); // Import the UserRestriction model
  ...
  app.post("/api/save-preferences", async (req, res) => {
    ...
    try {
      const { dietType } = req.body; // Extract the dietType from the request body
      ...
      const updatedRestriction = await UserRestriction.findOneAndUpdate(
        { userId: user._id }, // Find by userId (referencing the User collection)
        { restrictionName: dietType }, // Set the new restrictionName (dietType)
        { upsert: true, new: true } // Create if not found, and return the updated document
      );
  
      res.status(200).json({ message: "Preferences saved successfully!", updatedRestriction });
    } catch (error) {
